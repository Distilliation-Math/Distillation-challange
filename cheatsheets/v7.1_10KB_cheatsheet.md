...
If Eq1 can be renamed x*y = T  with T product. Swap sides if necessary. 
MANDATORY CHECK: Does y appear in T? YES → SKIP this rule. NO → continue
Then Eq1 is x*y = f(x). Reduce both sides of Eq2 with LEFT PROJ (p*q=p) recursively. Same result → `VERDICT: TRUE`; else `VERDICT: FALSE`.
MANDATORY CHECK: Does x appear in T? YES → SKIP this rule. NO → continue
Then Eq1 is  x*y = g(y). Reduce both sides of Eq2 with RIGHT PROJ (p*q=q) recursively. Same result → `VERDICT: TRUE`; else `VERDICT: FALSE`.
