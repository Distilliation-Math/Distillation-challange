...
2.1 Exact identity
If Eq2 is identical to Eq1 up to consistent variable renaming, answer TRUE.
RULE: exact renaming

2.2 Tautological target
If Eq2 is of the form x = x, answer TRUE.
RULE: tautological target

2.3 Singleton / collapse source
If Eq1 has the form x = T where x does not appear anywhere in T, then Eq1 forces a singleton magma. Answer TRUE. Side-swapped T = x also counts.
Example: Eq1 = x = y*z → x not in y*z → singleton → TRUE for any Eq2.
RULE: singleton collapse

2.4 Constant-operation source
If Eq1 and Eq2 have the form A = B where both A and B are non-variable product terms, and the variable sets of A and B are disjoint, then all products equal one constant. Answer TRUE.
...
C2: rhsVars=3, rhsTotals=112, Lx=FALSE, Rx=FALSE, xTop=right, topShape=v-m → TRUE
C3: rhsVars=4, rhsTotals=1112, Lx=FALSE, Rx=FALSE, xTop=right, topShape=v-m → TRUE
C4: rhsVars=3, xTop=right, topShape=v-m, xCount=2, rhsTotals=122, Rx=FALSE → TRUE
C5: rhsVars=3, xTop=right, topShape=v-m, xCount=2, rhsTotals=112 → TRUE
C6: rhsVars=3, xTop=right, topShape=v-m, xCount=2, rhsTotals=123 → TRUE
C7: rhsVars=3, Lx=FALSE, Rx=FALSE, xTop=left, topShape=m-v, xCount=2 → TRUE
C8: rhsVars=3, rhsTotals=113, Lx=FALSE, Rx=FALSE, xTop=right, topShape=v-m → TRUE
C9: rhsVars=3, rhsTotals=113, Lx=FALSE, Rx=FALSE, xTop=left, topShape=m-v → TRUE
C10: rhsVars >= 4 AND Lx=FALSE → TRUE
...
- x = (x*y)*x, x = ((x*y)*z)*x
  and Eq2 preserves the leading x while reshaping, answer TRUE.
  RULE: projection family

  2.10 Square-reduction family
  If Eq1 is x*x = x, x*(x*x) = x, or (x*x)*x = x, and the target is obtained by replacing a repeated x-block by x, answer TRUE.
  RULE: square reduction

  2.11 Direct exact instance
  If Eq2 is exactly a substitution instance or one-hole context instance of Eq1, answer TRUE.
  RULE: exact instance
  ...
  T1: If Eq1 is bare with vars(Eq1) >= 3 and size(Eq2) > size(Eq1), answer TRUE.
  (size = total variable occurrences on both sides)
  T4: If Eq1 is bare with vars(Eq1) >= 4 and vars(Eq2) = 2, answer TRUE.
...
Example: Eq1 x=((x*y)*z)*w (left-spine LLL, depth 3), Eq2 y=(y*z)*w (left-spine LL, depth 2). 3 does not divide 2 → FALSE.
