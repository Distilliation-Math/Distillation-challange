...
Read both equations as binary trees. Compute features:
- leftmost/rightmost variable = first/last variable in full leaf-reading
- WORD(term) = full left-to-right leaf list
- PARITY(term) = odd/even count of each variable
- SET(term) = distinct variables on EACH SIDE separately (LHS vars vs RHS vars, NOT whole-equation vars)
  Ex: x = x*(y*z) → LHS={x}, RHS={x,y,z} → SET match=F
- AB(term) = exact count of each variable per side (multiset)
- bare = one side is a single variable (see 2.5)
...
- RIGHT PROJECTION: every product collapses to its rightmost variable
  ...
  Apply if one side of Eq1 is a single variable (bare — x in product OK) and no earlier TRUE route fired.
...
If any motif below matches → TRUE immediately:
...
S1: LP(A)=T ∧ LP(B)=F → FALSE  [leftmost-variable: same on both sides of A, different on B]
S2: RP(A)=T ∧ RP(B)=F → FALSE  [rightmost-variable: same on both sides of A, different on B]
S3: WORD(A)=T ∧ WORD(B)=F → FALSE  [WORD match: same leaf-list both sides of A, different on B]
S4: PARITY(A)=T ∧ PARITY(B)=F → FALSE  [parity match per variable]
S5: SET(A)=T ∧ SET(B)=F → FALSE  [same variable set on each side of A, different on B]
S6: AB(A)=T ∧ AB(B)=F → FALSE  [exact variable counts match per side]
